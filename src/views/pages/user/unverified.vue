...
export default {
  ...
  computed: {
    ...mapGetters({
      unverifiedUsersRecord: "users/getUnverifiedUsers",
    }),
    /**
     * Total no. of records
     */
    rows() {
      return this.unverifiedUsersRecord.length;
    },
  },
  mounted() {
  ...
  methods: {
    ...mapMutations({
      populateUnverifiedUser: "users/SET_UNVERIFIED_USERS",
    }),
    gotoNext() {
    ...
    fetchData(page = 1) {
      if (this.unverifiedUsersRecord.length / 50 < page) {
        if (page === 1) {
          this.isBusy = !this.isBusy;
        }
        this.axios
          .get(BASE_URL + `/admin/users/unverified?page=${ page }&per_page=50`, {})
          .then((res) => {
            ...
            this.populateUnverifiedUser(userArr);
            // this.fetchRoles();
          })
          .catch((err) => {
            console.log(err);
          ...
          >
            ...
            <template v-slot:cell(action)="{ item }">
              <ul class="list-inline mb-0">
                ...
                <li
                  v-if="item.status == 'restricted' || item.status == 'pending'"
