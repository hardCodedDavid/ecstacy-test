...
export default {
  ...
  computed: {
    ...mapGetters({
      verifiedUsersRecord: "users/getVerifiedUsers",
    }),
    /**
     * Total no. of records
     */
    rows() {
      return this.verifiedUsersRecord.length;
    },
  },
  mounted() {
  ...
  methods: {
    ...mapMutations({
      populateVerifiedUser: "users/SET_VERIFIED_USERS",
    }),
    gotoNext() {
    ...
    fetchData(page = 1) {
      if (this.verifiedUsersRecord.length / 50 < page) {
        if (page === 1) {
          this.isBusy = !this.isBusy;
        }
        this.axios
          .get(BASE_URL + `/admin/users/verified?page=${ page }&per_page=50`, {})
          .then((res) => {
            ...
            this.populateVerifiedUser(userArr);
            // this.fetchRoles();
          })
          .catch((err) => {
