...
export default {
  ...
  computed: {
    ...mapGetters({
      usersRecord: "users/getUsers",
    }),
    /**
     * Total no. of records
     */
    rows() {
      return this.usersRecord.length;
    },
  },
  mounted() {
  ...
  methods: {
    ...
    approveUser(id) {
      this.isBusy = true;
      this.axios
        .put(BASE_URL + "/admin/user-action/" + id + "/approve")
        .then(() => {
    ...
    restrictUser(id) {
      this.isBusy = true;
      this.axios
        .put(BASE_URL + "/admin/user-action/" + id + "/restrict")
        .then(() => {
          // console.log(res.data.data);
          this.fetchData();
          this.$refs.mytoast.Add({
          ...
          >
            ...
            <template v-slot:cell(action)="{ item }">
              <ul class="list-inline no-wrap mb-0">
                ...
                <li
                  v-if="
                    item.status == 'unverified' || item.status == 'restricted'
                  "
