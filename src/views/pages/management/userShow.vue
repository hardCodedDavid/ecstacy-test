...
export default {
  ...
  methods: {
    getUser(record) {
      if (record.user) {
        return record.user;
      } else if (!record.user && record.receiver) {
    ...
    fundUserWallet() {
      this.isBusy = true;
      this.axios
        .put(BASE_URL + "/admin/wallet-action/credit", this.wallet)
        .then(() => {
          this.fetchData();
    ...
    withdrawUserFund() {
      this.isBusy = true;
      this.axios
        .put(BASE_URL + "/admin/wallet-action/debit", this.wallet)
        .then(() => {
          this.fetchData();
          this.$refs.mytoast.Add({
        ...
        this.axios
          ...
          .then((res) => {
            ...
            res.data.data.wdtransactions.map((item) => {
              ...
              u.bank_name =
                item.meta?.bank_name || "Not available";
              ...
              u.account_name =
                item.meta?.account_name || "Not available";
              u.account_number =
                item.meta?.account_number || "Not available";
